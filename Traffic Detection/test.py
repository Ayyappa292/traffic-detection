# -*- coding: utf-8 -*-
"""
Created on Fri Jun  3 10:55:09 2022
...
#gray=cv2.Canny(image,25,255,L2gradient=False)#cv2.Canny(grad,100,200,L2gradient=Tr  ue) 
#cv2.imwrite("gray/"+fdata,img) 
img = mpimg.imread('images/D.png')  
img = rgb2gray(img)  
imgs.append(img)

t = Window(imgs, sigma=1.4, kernel_size=5, lowthreshold=0.09, highthreshold=0.20,  weak_pixel=100) 
imgs = t.detect() 
for i, img in enumerate(imgs):  
    if img.shape[0] == 3: 
        img = img.transpose(1,2,0)  
        cv2.imwrite("gray/D.png",img) 
        img = mpimg.imread('refrence.png')  
        img = rgb2gray(img) 
        imgs.append(img) 
        t=Window(imgs,sigma=1.4, kernel_size=5, lowthreshold=0.09, highthreshold=0.20,  weak_pixel=100) 
        imgs = t.detect() 
